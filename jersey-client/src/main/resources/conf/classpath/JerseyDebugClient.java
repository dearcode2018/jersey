...
public class JerseyDebugClient
{
	...
	public void downloadB()
	{
	    ...
	    response = builder.get(ClientResponse.class);
	    final InputStream inputStream = response.getEntity(InputStream.class);
	   // final InputStream inputStream = builder.get(InputStream.class);
	    // 从中获取文件名 [attachment;filename=pom-full.xml]
	    String contentDisposition = response.getHeaders().get("Content-Disposition").get(0);
	    System.out.println("contentDisposition = " + contentDisposition);
	    String keyWord = "filename=";
	...
	{
		init();
	   // final FileDataBodyPart part = builder.get(FileDataBodyPart.class);
	  
	   // response = builder.get(ClientResponse.class);
	   // System.out.println(response.getEntity(File));
	    // 从中获取文件名 [attachment;filename=pom-full.xml]
	    //System.out.println(part.getFileEntity().getName());
	    try
		{
			final OutputStream outputStream = new FileOutputStream(param.getDownloadPath() + "/download.txt");
			//IoUtil.transport(inputStream, outputStream);
		} catch (FileNotFoundException e)
		{
			e.printStackTrace();
